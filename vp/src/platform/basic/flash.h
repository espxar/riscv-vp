...
#include <fcntl.h>
#ifdef __APPLE__
#include <sys/stat.h>  // macOS 相应的头文件
#include <fcntl.h>     // macOS 打开文件的头文件
#else
#include <linux/fs.h>
#endif
#include <stdint.h>
...
struct Flashcontroller : public sc_core::sc_module {
	...
	Flashcontroller(sc_module_name, string& filepath) : blockBuf(nullptr), mFilepath(filepath), mFiledescriptor(-1) {
       ...
       // 使用 fstat 获取文件大小
        ...
        mDeviceNumBlocks.asInt = st.st_size;  // 获取文件大小
        if (mDeviceNumBlocks.asInt <= 0) {
            cerr << "Invalid size of device: " << mFilepath << endl;
            close(mFiledescriptor);
            mFiledescriptor = -1;
            return;
        }
        ...
        // 计算块数
        cerr << "Device size: " << mDeviceNumBlocks.asInt << endl;
